 ...
 * 
 * <p>This class extends JUnit's {@link ExternalResource} to provide automatic setup
 * and teardown of a fake HTTP server that mimics the Limelight REST API. It listens
 * on port 5807 and handles two endpoints:
 * <ul>
 *   <li>{@code /results} - GET endpoint that returns vision data in JSON format</li>
 *   <li>{@code /upload-fieldmap} - POST endpoint that accepts field map uploads</li>
 * </ul>
 * 
...
public class FakeLimelight extends ExternalResource {
  ...
  /**
   * Sets up the fake Limelight server before each test.
   * Creates an HTTP server on port 5807 with endpoints for results and field map uploads.
   * 
  ...
  /**
   * Tears down the fake Limelight server after each test.
   * Stops the server with a 2-second grace period and clears the server reference.
   */
  @Override
   ...
   * 
   * <p>This handler responds to GET requests with a configurable JSON body
   * representing vision data from the Limelight. It returns 405 Method Not Allowed
   ...
   * 
   * <p>This handler accepts POST requests with field map data and stores the
   * uploaded content for verification in tests. It returns 405 Method Not Allowed
  ...
  private static class FakeFieldMap implements HttpHandler {
    ...
    /**
     * Handles incoming HTTP requests to the /upload-fieldmap endpoint.
     * Reads and stores the POST body content for later retrieval.
     * 
     * @param exchange the HTTP exchange containing request and response
  ...
  /**
   * Resets the fake Limelight state by clearing the results response body.
   * Useful for ensuring a clean state between test cases.
   */
  public void reset() {
