...
public abstract class VisionSubsystem<C extends Camera> extends SubsystemBase {
  ...
  public void periodic() {
    ...
    Pose2d drivetrainPose = getDrivetrainPose();
    if (drivetrainPose != null) {
      poseEstimator.publishCameraPosesRelativeTo(drivetrainPose);
    } else {
      DriverStation.reportError(
          "getDrivetrainPose() returned null! Possibly unintended behavior may occur!",
  ...
  protected void setPose(Pose2d pose) {
    if (pose != null) {
      if (poseEstimator.poseStrategyRequiresHeadingData()) {
        poseEstimator.resetHeadingData(Timer.getTimestamp(), pose.getRotation());
      }
    } else {
      DriverStation.reportError(
