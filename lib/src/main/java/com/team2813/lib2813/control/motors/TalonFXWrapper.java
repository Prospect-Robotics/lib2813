...
/**
 * Wrapper class for CTRE Phoenix 6 TalonFX motor controller.
 *
 * <p>This class provides a standardized interface for controlling TalonFX motor controllers while
 * maintaining compatibility with the team's control system architecture. It implements the {@link
 * PIDMotor} interface and supports advanced control modes including velocity control, motion magic,
 * and position control with the integrated encoder.
 *
 * <p>Key features include:
 *
 * <ul>
 *   <li>Type-safe unit support using WPILib's units system
 *   <li>Advanced control modes: duty cycle, voltage, velocity, and motion magic
 *   <li>PID control configuration with multiple slot support
 *   <li>Follower motor management with strict and regular following modes
 *   <li>Built-in current limiting for motor protection (40A stator limit)
 *   <li>Input validation for CAN IDs and invert types
 * </ul>
 *
 * <p>The wrapper automatically configures current limits (40A stator current limit with supply
 * current limiting enabled) and validates all input parameters to prevent configuration errors.
 *
 * @author Team 2813
 * @since 1.0
 */
public class TalonFXWrapper implements PIDMotor {
   ...
   *
   * <ul>
   *   <li>Motor output inversion based on the specified {@code invertType}
   *   <li>Current limits: 40A stator current limit with supply current limiting enabled
   *   <li>Input validation for CAN ID and invert type
   * </ul>
   *
   ...
   *     InvertType#rotationValues}. This exception is thrown when passed an {@link InvertType} that
   *     is for following motors
   ...
   *
   * <p>This is a convenience constructor that creates a TalonFX on the default "rio" CAN bus. The
   * constructor automatically configures the same settings as the primary constructor.
   *
   * @param canID the CAN ID of the motor controller, must be in range [0, 62]
   * @param invertType the inversion type for motor output, must be a rotation value
   ...
   *
   * <ul>
   *   <li>{@code VELOCITY}: Velocity control in rotations per second with optional feedforward
   *   <li>{@code MOTION_MAGIC}: Motion magic position control with trapezoidal motion profiling
   *   <li>{@code VOLTAGE}: Direct voltage control (demand in volts)
   *   <li>{@code DUTY_CYCLE} (default): Duty cycle control (demand as fraction from -1.0 to 1.0)
   * </ul>
   *
   * <p>Feedforward is supported for velocity and motion magic control modes to improve tracking
   * performance and reduce steady-state error.
   *
   * @param controlMode the control mode to use
   * @param demand the demand value (interpretation depends on control mode)
   ...
   * converted to other angular units as needed. The TalonFX uses its integrated encoder for
   ...
   *
   * <p>Stator current represents the current flowing through the motor windings and is useful for
   * monitoring motor load, detecting mechanical binding, and ensuring the motor stays within safe
   * operating limits.
   *
   ...
   * for the TalonFX's integrated encoder.
   *
   ...
   * second, which can be converted to other angular velocity units as needed.
   *
   ...
   *
   * <p>This method allows access to Phoenix-specific configuration and methods that are not exposed
   * through the {@link PIDMotor} interface, such as advanced motion profiling parameters and
   ...
   *
   * <ul>
   *   <li>{@code NeutralModeValue.Brake}: Motor actively resists motion when disabled
   *   <li>{@code NeutralModeValue.Coast}: Motor freewheels when disabled
   * </ul>
   *
   ...
   *
   * <p>The TalonFX supports multiple PID slot configurations that can be switched between during
   * operation. This allows for different control parameters for different operating conditions
   * (e.g., different PID gains for velocity vs position control).
   *
   * <p><b>Note:</b> The 'f' parameter represents velocity feedforward (kV) in Phoenix 6, not
   * traditional feedforward gain.
   *
   ...
   *
   * <p>This is a convenience method that calls {@link #configPIDF(int, double, double, double,
   * double)} with a velocity feedforward value of 0.
   *
   * @param slot the closed-loop slot to configure
   * @param p the proportional gain (kP)
   * @param i the integral gain (kI)
   ...
   * independent inversion settings. The wrapper supports both strict following and regular
   * following modes based on the invert type:
   *
   * <ul>
   *   <li><b>Strict Following:</b> Used when {@code invertType} is a rotation value
   *       (CLOCKWISE/COUNTER_CLOCKWISE). The follower uses hardware-level strict following with the
   *       specified absolute inversion.
   *   <li><b>Regular Following:</b> Used when {@code invertType} is FOLLOW_MASTER or OPPOSE_MASTER.
   *       The follower uses regular following with relative inversion.
   * </ul>
   *
   * <p>The follower TalonFX object is retained in the followers list to prevent garbage collection
   * and maintain the hardware relationship.
   *
   ...
   *
   * <p>This is a convenience method that calls {@link #addFollower(int, String, InvertType)} with
   * the default CAN bus. See that method for detailed behavior documentation.
   *
   * @param deviceNumber the CAN ID of the follower TalonFX, must be in range [0, 62]
   * @param invertType the inversion type for the follower motor
   * @throws InvalidCanIdException if the CAN ID is invalid (outside range [0, 62])
